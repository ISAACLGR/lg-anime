...
export default function PlayerScreen() {
  ...
  const normalizeQualityList = (sources: any[] = []) => {
    ...
    sources.forEach((source) => {
      ...
      if (!seen.has(key)) {
        seen.set(key, { ...source, label });
      }
    });

    return Array.from(seen.values()).sort((a, b) => parseQualityValue(b.label) - parseQualityValue(a.label));
  };
...
const WebVideoPlayer = ({ videoUrl, title, episodeUrl, contentType }: { videoUrl: string; title?: string; episodeUrl?: string; contentType?: string }) => {
...
const hlsPlaylist = isHlsStream(videoUrl) || isHlsStream(proxyVideoUrl) || (contentType ? contentType.toLowerCase().includes('mpegurl') : false);
  ...
  useEffect(() => {
    ...
    const existingScript = document.querySelector('script[data-hls-loader="animefire"]') as HTMLScriptElement | null;
    if ((window as any).Hls && (window as any).Hls.isSupported()) {
      ...
      script.src = 'https://cdn.jsdelivr.net/npm/hls.js@1.6.12/dist/hls.min.js';
      script.async = true;
  ...
  if (Platform.OS === 'web') {
    return (
      <div style={{ width: '100%', height: '100%', backgroundColor: '#000', position: 'relative', minHeight: '240px' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10000, padding: '12px', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', pointerEvents: 'none' }}>
          <span style={{ color: 'white', fontWeight: 'bold', fontSize: '18px' }}>{title || ''}</span>
        </div>
        <video
          ref={videoRef}
          controls
          ...
          playsInline
          preload="auto"
          style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'contain', zIndex: 1, backgroundColor: '#000' }}
        />
      </div>
    );
  }
  ...
  return (
    <WebView
      ...
      onError={(syntheticEvent) => { const { nativeEvent } = syntheticEvent; console.warn('❌ WebView error:', nativeEvent); }}
      onHttpError={(syntheticEvent) => { const { nativeEvent } = syntheticEvent; console.warn('❌ WebView HTTP error:', nativeEvent); }}
        ...
        if (tokenMatch) {
          ...
          for (const url of possibleUrls) {
            try {
              ...
              console.log('❌ Falha ao testar URL:', url, getErrorMessage(error));
              continue;
            }
          }
        }
...
const extractVideoData = async () => {
  try {
    ...
    const response = await fetch(`${effectiveApiBaseUrl}/api/animefire/extractVideo?url=${encodeURIComponent(episodeUrl)}`);
    if (!response.ok) {
    ...
    const data = await response.json();
      ...
      if (data.error || !data.success) {
        console.warn('🎬 fallback video activated because extraction failed:', data.error || 'Falha ao extrair vídeo');
        ...
        const normalizedQualities = normalizeQualityList(data.allQualities || []);
        ...
        if (iframeSrc && !videoUrl) {
            ...
            if (extractedVideo) {
              ...
              if (extractedVideo.includes('blogger.com') || extractedVideo.includes('iframe')) {
                ...
                setVideoData({
                  ...
                  contentType: data?.metadata?.accessibility?.contentType || 'application/vnd.apple.mpegurl'
                });
                setSelectedSourceIndex(0);
              } else {
                ...
                if (Platform.OS !== 'web') {
                  ...
                  setVideoData({
                    ...
                    contentType: data?.metadata?.accessibility?.contentType || (extractedVideo.toLowerCase().includes('m3u8') || extractedVideo.toLowerCase().includes('h.jpg') ? 'application/vnd.apple.mpegurl' : 'video/mp4')
                  });
                  setSelectedSourceIndex(0);
                }
              }
          ...
          if (Platform.OS !== 'web' && iframeSrc) {
            ...
            setVideoData({
              ...
              contentType: data?.metadata?.accessibility?.contentType || (videoUrl.toLowerCase().includes('m3u8') || videoUrl.toLowerCase().includes('h.jpg') ? 'application/vnd.apple.mpegurl' : 'video/mp4')
            });
            setSelectedSourceIndex(0);
          }
        } else {
          ...
          <View className="bg-black aspect-[4/3] max-h-64 relative">
            ...
            {hasRealVideo ? (
              <WebVideoPlayer 
                ...
                contentType={videoData?.contentType || videoData?.metadata?.accessibility?.contentType}
              />
          ...
          <View className="px-4 py-4">
            ...
            {videoData?.allQualities && videoData.allQualities.length > 1 && (
              <View className="mb-4">
                ...
                >
                  {videoData.allQualities.map((source: any, index: number) => (
                    <TouchableOpacity
                      key={index}
