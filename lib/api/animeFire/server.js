...
app.get('/filter-service/stats', async (req, res) => {
    try {
        const stats = await clienteAnimeFire.getCacheStats();
        res.json({
            success: true,
...
app.post('/filter-service/clear-cache', async (req, res) => {
    try {
        await clienteAnimeFire.clearCache();
        res.json({
            success: true,
